...
function fillCarousel(containerId, data) {
...
function initArrowButtons() {
    ...
    nextBtn?.addEventListener('click', () => container.scrollBy({ left: cardWidth + gap, behavior: 'smooth' }));
    prevBtn?.addEventListener('click', () => container.scrollBy({ left: -(cardWidth + gap), behavior: 'smooth' }));
}

// ===== MODAL CON RESULTADOS FILTRADOS =====
function mostrarModal(list) {
    const modal = document.getElementById('modal');
    const info = document.getElementById('modal-info');
    const res = document.getElementById('modal-results');

    if (!modal || !info || !res) return;
